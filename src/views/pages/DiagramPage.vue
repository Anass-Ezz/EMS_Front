...
const fuelMeterData = ref({
  'fm-0': {
    ...
    pressure: 4.2,      // bar - after pump
    flow: 8,            // L/s - for chaudière
    state: false        // connection state
  }
})
...
const startDrag = (event) => {
  if (event.target.closest('[data-node]') || event.target.closest('[id^="sm-"]')) {
    ...
    if (elementGroup) {
      ...
      if (textDiv) {
        if (isStateElement && isMeter) {
          ...
          // Optional: Style power text differently if needed
          // textDiv.style.color = '#ffffff'; // Example: white text for power
        } else if (isGasFuel) {
          // Gas/Fuel meter data
...
const animateGasFuelValues = () => {
  ...
  fuelMeterData.value['fm-0'].pressure = fluctuateGasFuelValue(
    fuelMeterData.value['fm-0'].pressure, 3.8, 4.8, 0.05
  ) // ±5% variation, 3.8-4.8 bar (after pump)
  
  fuelMeterData.value['fm-0'].flow = fluctuateGasFuelValue(
    fuelMeterData.value['fm-0'].flow, 6, 10, 0.03
  ) // ±3% variation, 6-10 L/s
}
...
const startPowerAnimation = () => {
  ...
  powerAnimationId = setInterval(() => {
    animatePowerValues()
  }, 1000) // Update every second like Sankey chart
}
