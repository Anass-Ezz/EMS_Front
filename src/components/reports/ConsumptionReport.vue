<template>
      ...
      <div class="flex items-start justify-between mb-4">
        <div>
          ...
          <p class="text-gray-400 text-sm">
            ...
            <span v-if="noteSynthetic" class="ml-2 text-xs text-amber-400">(Includes synthetic gas & fuel for demo)</span>
          </p>
        </div>
  ...
  const METER_CONFIG = {
    'sm-a-0': { name: 'WS-A Stamping Press', base: 450, volatility: 0.15, spiky: true },
    'sm-a-1': { name: 'WS-A Induction Heater', base: 420, volatility: 0.1, spiky: true },
    'sm-a-2': { name: 'WS-A Coil Feeder', base: 85, volatility: 0.08, spiky: false },
    'sm-a-3': { name: 'WS-A Large Welder', base: 42, volatility: 0.25, spiky: true },
    'sm-a-4': { name: 'WS-A Controls', base: 22, volatility: 0.03, spiky: false },
    'sm-a-5': { name: 'WS-A Utilities', base: 27, volatility: 0.05, spiky: false },
    'sm-b-0': { name: 'WS-B Chassis Mounting', base: 90, volatility: 0.12, spiky: true },
    'sm-b-1': { name: 'WS-B Robotic Arms', base: 72, volatility: 0.2, spiky: true },
    'sm-b-2': { name: 'WS-B Wiring Benches', base: 22, volatility: 0.05, spiky: false },
    'sm-b-3': { name: 'WS-B Control', base: 24, volatility: 0.03, spiky: false },
    'sm-b-4': { name: 'Solar Inverter (PV)', base: 120, volatility: 0.05, spiky: false }
  }
  ...
  function computeAggregations() {
    ...
    tableMeters.value = ELECTRIC_METERS
      .filter(id => id !== PV_METER_ID && meterTotals[id])
  ...
  function buildCharts() {
    ...
    chartPower.value = {
      ...
      series: [
        { name: 'WS-A High Load', type: 'line', stack: 'Total', areaStyle: { opacity: 0.7 }, smooth: true, showSymbol: false, data: createSeries(idsAHigh) },
        ...
        { name: 'PV (Generation)', type: 'line', smooth: true, showSymbol: false, lineStyle: { type: 'dashed', color: '#fbbf24' }, data: createSeries([PV_METER_ID]) }
      ]
    }
    ...
    chartBreakdown.value = {
      ...
      series: [{
        ...
        data: [
          { name: 'WS-A High', value: sumKWh(idsAHigh) },
          { name: 'WS-A Other', value: sumKWh(idsAOther) },
          { name: 'WS-B', value: sumKWh(idsB) },
          { name: 'PV (Generated)', value: kpis.value.pvKWh }
        ]
      }]
    }
  }
  ...
  function aggregateDailyEnergy() {
    const map = new Map(), ids = ELECTRIC_METERS.filter(id => id !== PV_METER_ID)
