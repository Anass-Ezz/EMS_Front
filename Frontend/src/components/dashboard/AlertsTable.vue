    ...
    <template #content>
      ...
      <div v-else-if="currentView === 'list'" class="flex-1 h-[365px] overflow-y-auto">
        <div class="divide-y divide-gray-700">
          ...
          <!-- No edges message -->
          <div v-if="displayedEdges.length === 0" class="text-center py-8 text-gray-400">
            No edges found
          </div>
        </div>
      </div>
...
// Computed properties for displayed edges (always 4 edges)
const displayedEdges = computed(() => {
  // Always show exactly 4 edges: edge0, edge1, edge2, edge3
  const edgeIds = ['edge0', 'edge1', 'edge2', 'edge3']
  
  // Get the real edge0 data if available
  const realEdge0 = edges.value.find(e => e.id === 'edge0')
  
  return edgeIds.map(id => {
    if (id === 'edge0') {
      // Return real edge0 data if available, otherwise create a placeholder
      return realEdge0 || {
        id: 'edge0',
        isOnline: true,
      ...
      // Create static copies for edge1, edge2, edge3
      return {
        id,
        isOnline: true,
      }
...
const sortedEdges = computed(() => {
  ...
  const edgesCopy = [...displayedEdges.value]; // Use displayedEdges instead of raw edges
...
async function fetchEdgeStates() {
  // Only fetch states for edge0 (the real dynamic edge)
  const onlineEdgeIds = ['edge0'] // Only edge0 is dynamic
...
watch(() => getOverallState(), (newState) => {
  emit('update:globalState', newState)
}, { immediate: true })

// Component detail functionality
async function getEdgeConfig(edgeId) {
  // For edge0, fetch real config
  // For edge1-3, use the same config as edge0 but modify states
  if (edgeId === 'edge0') {
    await waitForSocketOpen(ws)
    ...
    // For static edges (edge1-3), use the cached config from edge0
    // But we need to wait until edge0 config is loaded
    if (Object.keys(factories).length === 0) {
      // If edge0 config isn't loaded yet, try to load it first
      await getEdgeConfig('edge0')
      // Then use it for the static edge
      applyStaticEdgeConfig(edgeId)
    } else {
      applyStaticEdgeConfig(edgeId)
    }
  }
...
function applyStaticEdgeConfig(edgeId) {
  ...
  // Copy edge0's config but set all states to OK (0)
  Object.assign(factories, { ...factories }) // Copy factories
  Object.entries(components).forEach(([n, c]) => {
    components[n] = { 
      alias: c.alias, 
      factoryId: c.factoryId, 
      state: 0, // Force OK state for static edges
      expanded: false, 
      lastUpdated: new Date().toLocaleString() 
    }
  })
  
  // Add _sum component if it exists
  if (components['_sum']) {
    components['_sum'].state = 0
  }
  
  loading.value = false
  
  // Don't start polling for static edges
...
async function fetchComponentStates() {
  ...
  // Only fetch states for edge0 (dynamic edge)
  if (selectedEdgeId.value !== 'edge0') return
...
function fetchComponentChannels(name) {
  // Only fetch channels for edge0 (dynamic edge)
  if (selectedEdgeId.value !== 'edge0') return
...
function goBackToEdgeList() {
  ...
  // Restart edge state polling (only for edge0)
  if (auth.ready) {
    fetchEdgeStates()
    if (!fetchEdgeStatesInterval) {
...
function handleMessage(evt) {
  ...
  if (msg.id === edgeReqId) {
    ...
    if (msg.result) {
      ...
      // Start fetching edge states after getting edges (only for edge0)
  ...
  if (msg.id === edgeStatesReqId && msg.result) {
    // Clear existing edge states before updating
    Object.keys(edgeStates.value).forEach(key => delete edgeStates.value[key]);
    
    // Only update edge0 state from real data
    if (msg.result['edge0'] && msg.result['edge0']['_sum/State'] !== undefined) {
      edgeStates.value['edge0'] = msg.result['edge0']['_sum/State']
    }
    
    // Set static edges to OK state
    edgeStates.value['edge1'] = 0
    edgeStates.value['edge2'] = 0
    edgeStates.value['edge3'] = 0
    
    return
  }
  
  // Handle edge config response (only for edge0)
  if (msg.id === configRequestId && msg.result) {
    ...
    // Only start polling for edge0
    if (selectedEdgeId.value === 'edge0') {
  ...
  // Handle polled component states response (only for edge0)
...
function getOverallState() {
  if (currentView.value === 'detail') {
    ...
    // Return highest state among the 4 displayed edges
    // edge0 uses real state, others are always 0 (OK)
    const edge0State = edgeStates.value['edge0'] || 0
    return edge0State
  }
}
