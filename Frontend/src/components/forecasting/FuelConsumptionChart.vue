...
const generateForecastData = () => {
  ...
  // Base fuel consumption patterns (lowest consumption)
  const baseConsumption = 30 // L base
  const dailyVariation = 0.2 // 20% variation
  const seasonalFactor = 1.0 + (Math.sin(Date.now() / (1000 * 60 * 60 * 24 * 30)) * 0.4) // Highest seasonal variation for fuel
  ...
  for (let i = 0; i < dataPoints; i++) {
    ...
    if (props.timeRange === 'day') {
      ...
      // Fuel consumption is more variable, especially during heating periods
      const hourFactor = 0.6 + (Math.sin((hour - 8) * Math.PI / 16) * 0.8) // Peak in morning and evening
      ...
      // Fuel consumption varies more on weekends (heating)
      const isWeekend = i >= 5
      const weekendFactor = isWeekend ? 1.2 : 0.9
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        params.forEach(param => {
          ...
          tooltipText += `${param.marker} ${param.seriesName}: ${value} L<br/>`
        })
        return tooltipText
      }
    },
