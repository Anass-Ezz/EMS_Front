...
const generateForecastData = () => {
  ...
  // Base gas consumption patterns (lower than electricity)
  const baseConsumption = 45 // kg base
  const dailyVariation = 0.25 // 25% variation
  const seasonalFactor = 1.0 + (Math.sin(Date.now() / (1000 * 60 * 60 * 24 * 30)) * 0.3) // Higher seasonal variation for gas
  ...
  for (let i = 0; i < dataPoints; i++) {
    ...
    if (props.timeRange === 'day') {
      ...
      // Gas consumption is more consistent throughout the day
      const hourFactor = 0.8 + (Math.sin((hour - 6) * Math.PI / 12) * 0.4) // Peak around noon
      
      // Add some randomness
      const randomFactor = 0.85 + Math.random() * 0.3
      
      forecast = baseConsumption * hourFactor * seasonalFactor * randomFactor
      ...
      // Gas consumption is more consistent across days
      const dayFactor = 0.9 + Math.random() * 0.2
      
      forecast = baseConsumption * 24 * seasonalFactor * dayFactor
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        params.forEach(param => {
          ...
          tooltipText += `${param.marker} ${param.seriesName}: ${value} kg<br/>`
        })
        return tooltipText
      }
    },
